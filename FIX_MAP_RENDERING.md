# 🏗️ FIX: Map Rendering — From Imperative to Declarative

## 1. Root Cause Analysis

### Why `map.geoObjects.add()` Failed

The `BeaconsSync` component used **imperative** Yandex Maps API calls (`map.geoObjects.add(placemark)`) inside a `useEffect`. This broke under React 18's Strict Mode lifecycle:

```
React 18 Strict Mode:
  mount → useEffect runs → adds placemarks to map
  unmount → cleanup runs → removes ALL placemarks
  mount (again) → useEffect runs → BUT timing race:
    - mapRef.current may be stale (old map instance)
    - ymaps may not be ready yet on the second mount
    - geoObjects.add() operates on a ghost reference
```

**The core conflict:** React's declarative model expects effects to be idempotent and safe to replay. But `map.geoObjects.add()` is a **side-effectful imperative mutation** — calling it twice after a cleanup doesn't restore state because:

1. The cleanup removes placemarks from the *previous* map instance
2. The re-mount tries to add to a *new* map instance via a stale ref
3. `useYMaps()` returns `null` on the first tick of the second mount, so the early return guard (`if (!ymaps)`) kills the entire effect

**Result:** The effect fires but bails out, leaving zero placemarks on the map.

---

## 2. The Fix: Declarative `<Placemark />` Components

Replace the entire imperative `BeaconsSync` component with React-native `<Placemark />` components rendered **inside** the `<Map>` tree. The library handles mount/unmount/update automatically.

### Complete `map-view.tsx` Replacement
...
export function MapView() {
  ...
  return (
    <div className="relative w-full h-screen bg-black">
      ...
      >
        <div
          className="yandex-map-dark w-full h-full"
...
| Before | After |
|--------|-------|
| `BeaconsSync` component with `useEffect` + `map.geoObjects.add()` | Deleted entirely |
| `mapInstanceRef` used to imperatively access map | Removed (no longer needed) |
| `useYMaps()` hook dependency | Removed (no longer needed) |
| `<Map />` is self-closing `<Map ... />` | `<Map>` wraps `<Placemark>` children |
| Placemarks managed imperatively | `validBeacons.map()` renders `<Placemark>` declaratively |

### Key Design Decisions

1. **`<Placemark>` is a child of `<Map>`** — the library requires this. Placemarks auto-register.
2. **`validBeacons` via `useMemo`** — filters invalid coords once, prevents re-renders.
3. **`geometry={[lat, lng]}`** — Yandex Maps uses `[lat, lng]` order (NOT `[lng, lat]`).
4. **`onClick` prop** — the `AnyObject` spread type on `Placemark` allows any event handler.
5. **Neon styling preserved** — `preset: "islands#circleIcon"` + `iconColor: "#00f3ff"` + CSS animation via `.yandex-map-dark [class*="image-with-content"]`.
6. **`iconContent: emoji`** — uses the `islands#circleIcon` preset which renders `iconContent` inside.
